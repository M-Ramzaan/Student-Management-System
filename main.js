...
class Student {
    ...
    payFees(fees) {
        if (fees <= 0) {
            console.log(chalk.red(`Invalid fee amount: ${fees}. Please enter a positive value.`));
        }
        else if (this.balance < fees) {
            console.log(chalk.red(`Insufficient balance to pay $${fees} fees for ${this.name}. Current balance: $${this.balance}`));
        }
        else {
            this.balance -= fees;
            console.log(`${fees} fees paid successfully for ${this.name}.Current balance: $${this.balance}`);
        }
    }
...
class studentManager {
    ...
    addStudent(name) {
        ...
        console.log(`Student ${name} is added successfully. This student ID: ${student.id}`);
    }
    // Method to enroll a student in a course
    enrollStudent(studentId, studentCourse) {
        let student = this.findStudent(studentId);
        if (student) {
            student.enrollCourse(studentCourse);
            console.log(`${student.name} is enrolled in ${studentCourse} successfully.`);
        }
    }
    ...
    viewStudentBalance(studentId) {
        ...
        if (student) {
            ...
            console.log(chalk.red("Sorry student not found. Please check and reenter a correct student id."));
        }
    }
    // Method to find a student fees
    payStudentFees(studentId, feeAmount) {
        let student = this.findStudent(studentId);
        if (student) {
...
async function welcome() {
    let rainbowTitle = chalkAnimation.rainbow(`Welcome to Muhammad Ramzan Akram's "Student Management System".`);
    await sleep();
    rainbowTitle.stop();
}
await welcome();
    ...
    while (true) {
        ...
        switch (userInput.input) {
            ...
            case "Enroll Student":
                ...
                mainStudent.enrollStudent(Number(userInputEnroll.id), userInputEnroll.course);
                break;
            ...
            case "Pay Fees":
                ...
                mainStudent.payStudentFees(Number(userInputFees.id), Number(userInputFees.fees));
                break;
